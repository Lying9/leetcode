...
public class LeastInterval621 {
        ...
        while(count[count.length-1] > 0){
            ...
            while(i<=n){
                if(count[count.length-1] == 0)
                    break;
                if(i<count.length && count[count.length-i-1]>0)
   ...
   /* public static int leastInterval(char[] tasks, int n) {
       ...
       *//* result = (n+1)*(count[count.length-1]-1)+same;
        if(count.length > n+1){
            if(sum > result)
                result += (sum-result-1)*(n+1)+1;
        }*//*
       int result;
        result = compute(count,n);
        return result;

    }
    public static int compute(int[] count,int n){
        Arrays.sort(count);
        int sum = 0,result,same = 0;
        for (int i = 0; i < count.length ; i++) {
            sum += count[i];
        ...
        if(count.length > n+1){
            if(sum >= result+n+1-same){
                ...
                if(sub >0 ){
                    //count[i] -= sub;
                   *//* if(i>0){
                        if(count[i] - count[i-1] >= sub)
                            count[i] -= sub;
                        else{
                            count[i] -= (count[i] - count[i-1])+(sub-(count[i] - count[i-1]))/2;
                            count[i-1] -= sub - ((count[i] - count[i-1])+(sub-(count[i] - count[i-1]))/2);
                        }
                    }*//*
                   int k =i;
                    for (int j = 0; j <sub ; j++) {
                ...
                int[] subcount = new int[i+1];
                for (int j = 0; j <=i ; j++) {
                    subcount[j] = count[j];
                }
                result += compute(subcount,n);
            }
                //result += (sum-result-1)*(n+1)+1;
        }
        return result;
